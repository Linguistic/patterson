...
object JacksonPOJO {
  ...
  class JSONSentence() {
    ...
    def setBasicDependencies(basicDependencies: Array[JSONDependency]): Unit =
  ...
  class YAMLGrammarData() {
    ...
    def getMatcher: String = this.matcher
    def setMatcher(matcher: String): Unit = this.matcher = matcher
